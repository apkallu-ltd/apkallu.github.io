...
<header class="modern-header">
  <div class="container">
    <h1>コミュニティチャレンジ日程</h1>
    <p>期間が決まったチャレンジに参加しよう。目標を選び、毎日前進し、コミュニティの仲間と一緒にバッジを獲得しましょう。</p>
  </div>
</header>
...
<div class="container" style="padding-bottom: 80px;">
  ...
  {% if ongoing_count > 0 %}
    <h2 style="margin: 60px 0 30px; font-weight: 900; font-size: 2rem; color: #001A33; border-bottom: 4px solid #48BBFA; display: inline-block; padding-bottom: 10px;">開催中のチャレンジ</h2>
        ...
        {% if challenge_start <= current_time and challenge_end >= current_time %}
          <div class="modern-card">
            <div class="card-image" style="background-image: url('{{ challenge.image }}')">
              ...
              <div style="position: absolute; top: 15px; right: 15px; background: #FF9800; color: white; padding: 5px 12px; border-radius: 20px; font-size: 0.75rem; font-weight: bold; z-index: 2; box-shadow: 0 2px 8px rgba(0,0,0,0.15);">開催中</div>
            </div>
            <div class="card-content">
              ...
              <div class="card-footer">
                <span class="card-date">{{ challenge.start_date | date: "%Y年%m月%d日" }} 開始</span>
                <a href="{{ challenge.url | relative_url }}" class="card-link">詳細を見る →</a>
              </div>
            </div>
          </div>
        {% endif %}
  ...
  <!-- Upcoming Challenges -->
  {% if upcoming_count > 0 %}
    <h2 style="margin: 60px 0 30px; font-weight: 900; font-size: 2rem; color: #001A33; border-bottom: 4px solid #48BBFA; display: inline-block; padding-bottom: 10px;">近日開催のチャレンジ</h2>
    <div class="modern-grid">
      {% for challenge in sorted_challenges %}
        {% assign challenge_start = challenge.start_date | date: "%s" | plus: 0 %}
        {% if challenge_start > current_time %}
          <div class="modern-card">
            ...
            <div class="card-content">
              ...
              <p class="card-description">{{ challenge.description | truncate: 140 }}</p>
              <div class="card-footer">
                <span class="card-date">{{ challenge.start_date | date: "%Y年%m月%d日" }} 開始予定</span>
                <a href="{{ challenge.url | relative_url }}" class="card-link">詳細を見る →</a>
              </div>
            </div>
          </div>
  ...
  <!-- Past Challenges -->
  {% if past_count > 0 %}
    <h2 style="margin: 60px 0 30px; font-weight: 900; font-size: 2rem; color: #7f8c8d; border-bottom: 4px solid #bdc3c7; display: inline-block; padding-bottom: 10px;">終了したチャレンジ</h2>
      ...
      {% for challenge in sorted_challenges %}
        ...
        {% if challenge_end < current_time %}
          <div class="modern-card">
            ...
            <div class="card-content">
              <div class="card-tags">
                <span class="card-tag" style="background: #e0e0e0; color: #666;">終了</span>
              </div>
              ...
              <div class="card-footer">
                <span class="card-date" style="color: #888;">{{ challenge.end_date | date: "%Y年%m月%d日" }} 終了</span>
                <a href="{{ challenge.url | relative_url }}" class="card-link">詳細を見る →</a>
              </div>
            </div>
          </div>
  ...
  {% if ongoing_count == 0 and upcoming_count == 0 %}
    <div style="text-align: center; padding: 80px 20px; background: #fff; border-radius: 20px; box-shadow: 0 4px 20px rgba(0,0,0,0.05); margin-top: 60px;">
      ...
      <h3 style="font-weight: 800; font-size: 1.5rem; margin-bottom: 10px;">現在予定されているチャレンジはありません</h3>
      <p style="color: #64748b;">新しいコミュニティチャレンジを準備中です。ぜひアプリをダウンロードして、自分だけのチャレンジを作成してみてください！</p>
    </div>
  {% endif %}
